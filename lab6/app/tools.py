...
class ReviewOrderer:
    def __init__(self, order_by, direction, course_id, ignore):
        self.order_by = order_by
        self.direction = direction
        self.ignore = ignore
        self.query = db.select(Review).filter_by(course_id=course_id)
    ...
    def __order(self):
        ...
        elif self.order_by == "rating" and self.direction == "asc":
            self.query = self.query.order_by(Review.rating.asc())
        elif self.order_by == "rating" and self.direction == "desc":
            self.query = self.query.order_by(Review.rating.desc())
    
    def __ignore(self):
        if self.ignore:
            self.query = self.query.where(Review.id != self.ignore)
        pass
